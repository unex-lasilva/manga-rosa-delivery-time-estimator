...
public class BinaryTree <T extends Comparable <T>> implements Tree<T> {
    ...
    //Metodo recursivo para inserir um novo nó na árvore binária
    ...
    public void remove(T value) {
        this.root = removerNode(root, value);
        size--;
    }

    //Metodo recursivo para remover um nó da árvore binária
    private BinaryTreeNode<T> removerNode(BinaryTreeNode<T> node, T value) {
        if(node == null) {
            return null;
        }
        if(node.getValue().equals(value)) {
            ...
            node.setRightChild(removerNode(node.getRightChild(), smallestValue));
        } 
        else if(value.compareTo(node.getValue()) < 0) {
            node.setLeftChild(removerNode(node.getLeftChild(), value));
        }else{
            node.setRightChild(removerNode(node.getRightChild(), value));
        }
            return node;
        }
